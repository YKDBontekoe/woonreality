...
where stage in ('profile', 'shortlist', 'documents');

alter table public.purchase_cases alter column stage set default 'intake';

create table if not exists public.user_listings (
  user_id uuid not null references auth.users(id) on delete cascade,
  bag_vbo_id varchar(16) not null,
  source_url text,
  ...
  constraint user_listings_asking_price_non_negative check (asking_price is null or asking_price >= 0)
);

alter table public.user_listings enable row level security;
grant select, insert, update, delete on public.user_listings to authenticated;
drop policy if exists "users own listings" on public.user_listings;
create policy "users own listings" on public.user_listings for all to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
...
begin
  ...
  if p_payload ? 'stage' then
    ...
    if v_stage in ('profile', 'shortlist', 'documents') then
      v_stage := case v_stage when 'profile' then 'intake' when 'shortlist' then 'research' else 'research' end;
    end if;

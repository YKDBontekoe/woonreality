-- De "huidige woning" is een baseline voor vergelijken: geen koopdoel, dus
-- geen nieuwe stage in saved_properties maar een veld in preferences_json.
-- Atomair mergen zonder concurrente buyerProfile/mortgage/compare writes te
-- overschrijven, via dezelfde RPC die de rest van de workspace al gebruikt.
create or replace function public.merge_profile_preferences(
  ...
  p_current_home jsonb default null
)
returns jsonb
language plpgsql
security invoker
...
begin
  ...
  if p_onboarding is not null then v_patch := v_patch || jsonb_build_object('onboarding', p_onboarding); end if;
  if p_current_home is not null then v_patch := v_patch || jsonb_build_object('currentHome', p_current_home); end if;
...
grant execute on function public.merge_profile_preferences(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) to authenticated;

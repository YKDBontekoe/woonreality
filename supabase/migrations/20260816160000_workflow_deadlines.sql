-- Extend apply_case_workflow with the koopovereenkomst signing date and the
-- negotiated ontbindende-voorwaarden termijnen (financiering, bouwkundige
-- keuring), so the deadline engine (src/lib/deadlines.ts) can compute
-- bedenktijd and voorbehoud-vervaldatums for case_tasks.due_at.
...
begin
  ...
  v_touches_bid := p_payload ? 'offerAmount' or p_payload ? 'contractAmount' or p_payload ? 'financingCondition'
    ...
    or p_payload ? 'transferDate' or p_payload ? 'contractSignedAt' or p_payload ? 'financingWeeks' or p_payload ? 'inspectionWeeks';

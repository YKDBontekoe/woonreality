...
export function ValuationBidPanel({ bagId, analysis, listing }: { bagId: string; analysis: Analysis; listing: PropertyListing | null }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/property/${encodeURIComponent(bagId)}/bid-draft`, { cache: "no-store" })
      .then(async (response) => {
        if (response.status === 401) { setDraftResolved(true); return; }
      ...
      .catch((error) => { setDraftResolved(true); setSaveError(error instanceof Error ? error.message : "Bodconcept kon niet worden geladen."); });
  }, [bagId]);

...
export function PurchaseWorkflow({ caseId, initialStage }: { caseId: string; initialStage: string }) {
  ...
  useEffect(() => {
    fetch(`/api/cases/${encodeURIComponent(caseId)}/workflow`, { cache: "no-store" })
      .then(async (response) => {
      ...
      .catch((error) => setWorkflowError(error instanceof Error ? error.message : "Workflow kon niet worden geladen."));
  }, [caseId]);

  const range = useMemo(() => estimateBidRange(askingPrice), [askingPrice]);
  const currentIndex = Math.max(0, workflowSteps.findIndex(([key]) => key === initialStage));
  ...
  async function save(stage?: string) {
    const response = await fetch(`/api/cases/${encodeURIComponent(caseId)}/workflow`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ askingPrice: askingPrice || undefined, offerAmount: offerAmount || undefined, financingAmount: financingAmount || undefined, contractAmount: contractAmount || undefined, transferDate: transferDate || undefined, financingCondition, inspectionCondition, ...(stage ? { stage } : {}) }) });
    const body = await response.json() as { error?: string };
    if (!response.ok) throw new Error(body.error ?? "Workflow kon niet worden opgeslagen.");
    setWorkflowError(""); setSaved(true); window.setTimeout(() => setSaved(false), 1800);
  }

...
export function PurchaseWorkflow({ caseId, initialStage, analysis, bagVboId }: { caseId: string; initialStage: string; analysis?: Analysis | null; bagVboId?: string | null }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cases/${encodeURIComponent(caseId)}/workflow`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        ...
        setFinancingAmount(body.finance?.financing_amount ?? 0);
        ...
        if (body.bid?.conditions?.contractSignedAt) setContractSignedAt(body.bid.conditions.contractSignedAt);
        if (body.bid?.conditions?.financingWeeks != null) setFinancingWeeks(body.bid.conditions.financingWeeks);
        if (body.bid?.conditions?.inspectionWeeks != null) setInspectionWeeks(body.bid.conditions.inspectionWeeks);
      })
  ...
  const costs = useMemo(() => estimateBuyerCosts(offerAmount || askingPrice, workspace.buyerProfile, financingAmount || null), [askingPrice, financingAmount, offerAmount, workspace.buyerProfile]);

...
export function PropertyScoreCharts({
  ...
  const unscoredAttentionDomains = analysis.domains.filter((domain) => domain.hasUnscoredAttention);
    ...
    <section className="dash-score-charts">
      <div className="dash-score-card">
        ...
        <div className="domain-score-grid">
          ...
          <div className="domain-score-side">
            {unscoredAttentionDomains.length > 0 && (
              <p className="domain-attention-note">
                <AlertTriangle size={12} aria-hidden="true" />
                <span>
                  <strong>{unscoredAttentionDomains.map((domain) => domain.label).join(", ")}:</strong>{" "}
                  {t("charts.unscoredAttentionNote")}
                </span>
              </p>
            )}
            {mix.length > 0 && (

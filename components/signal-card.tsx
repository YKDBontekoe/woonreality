...
export function SignalCard({ signal }: { signal: Signal }) {
  ...
  return <article className="signal-card">
    <div className="signal-card-top"><div className="signal-title"><span className={`signal-dot ${signal.severity}`} /><h3>{signal.label}</h3></div><div className="signal-value">{value}{signal.unit && <small style={{ fontSize: 10, color: "var(--muted)" }}>{signal.unit}</small>}</div></div>
    <p className="signal-summary">{signal.summary}</p>
    <div className="signal-action"><strong>Check dit:</strong> {signal.action}</div>
    {score !== undefined && <div className="signal-bar"><div className={`signal-bar-fill ${signal.severity}`} style={{ width: `${Math.max(0, Math.min(100, score * 10))}%` }} /></div>}

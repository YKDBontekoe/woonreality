...
export function PropertyDashboard({ bagId }: { bagId: string }) {
  ...
  async function saveChecklist(next: ChecklistItem[]) {
    ...
    try {
      ...
      setChecklistError("");
    } catch (caught) { setChecklistError(caught instanceof Error ? caught.message : "Checklist kon niet worden opgeslagen."); }
  }
  ...
  return <main className="site-shell"><div className="container">
    ...
    <section className="checklist-section" id="checklist"><div className="section-inline-heading"><div><div className="eyebrow"><span className="eyebrow-dot" /> klaar voor de bezichtiging</div><h2>Jouw checklist</h2><p>Concrete vragen uit deze analyse, opgeslagen in je aankoopomgeving.</p>{checklistError && <p className="form-message" role="status">{checklistError} <Link href="/login">Inloggen</Link></p>}</div><button className="secondary-button" type="button" onClick={() => window.print()}><Printer size={14} /> Print / bewaar als PDF</button></div><div className="checklist-list">{checklist.map((item) => <label className={`checklist-item ${item.checked ? "checked" : ""}`} key={item.id}><input type="checkbox" checked={item.checked} onChange={(event) => { void saveChecklist(checklist.map((candidate) => candidate.id === item.id ? { ...candidate, checked: event.target.checked } : candidate)); }} /><span><strong>{item.label}</strong>{item.reason && <small>{item.reason}</small>}<input className="checklist-note" value={item.note ?? ""} placeholder="Eigen notitie (privé)" onChange={(event) => { void saveChecklist(checklist.map((candidate) => candidate.id === item.id ? { ...candidate, note: event.target.value } : candidate)); }} onClick={(event) => event.preventDefault()} /></span></label>)}<button className="add-checklist" type="button" onClick={() => { void saveChecklist([...checklist, { id: `custom-${Date.now()}`, label: "Eigen punt", checked: false }]); }}>+ Eigen punt toevoegen</button></div></section>

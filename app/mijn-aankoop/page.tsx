...
export default async function MyPurchasePage({ searchParams }: { searchParams: Promise<{ case?: string }> }) {
  ...
  return <><div className="container"><SiteHeader current="aankoop" /></div>{configured && <div className="container cockpit-account"><SignOutButton /></div>}<PurchaseCockpit initialCases={cases} /></>;
}

...
export default async function MyPurchasePage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ case?: string; setup?: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("mijn-aankoop");
  const search = await searchParams;
  if (search.case) redirect(`/mijn-aankoop/${search.case}`);
  ...
  if (clientConfigured) {
    ...
    account = { email: auth.user.email ?? t("emailFallback"), emailConfirmed: Boolean(auth.user.email_confirmed_at), suggestPasskey: search.setup === "passkey" };
    if (configured) {
      ...
      if (shouldRedirectToOnboarding({
        ...
        redirect(search.setup === "passkey" ? "/onboarding?setup=passkey" : "/onboarding");
      }

...
import { getCrimeContext, politieMisdrijvenTableUrl, type CrimeContext } from "@/src/lib/sources/politie";
...
export async function analyzeProperty(property: Property): Promise<Analysis> {
  ...
  const cbsEvidence = createCbsEvidence(cbs, cbsBuurtenUrl);
  const sesEvidence = createSesEvidence(ses, sesStatLineTableUrl);
  const crimeEvidence = createCrimeEvidence(crime, politieMisdrijvenTableUrl);
  ...
  const signals: Signal[] = [
    ...
    cbsContextSignal({ cbs, cbsEvidence }),

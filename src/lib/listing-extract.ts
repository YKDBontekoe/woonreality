...
export function factsFromFundaUrl(sourceUrl: string): ImportedListingFacts {
...
export function uniqueNotes(values: string[]) {
...
export function applyKenmerk(label: string, value: string, facts: ImportedListingFacts) {
  const key = label.toLowerCase().replace(/\s+/g, " ").trim();
  const text = value.replace(/\s+/g, " ").trim();
  if (!key || !text || PAYWALLED.test(text)) return;
  const extraCount = Object.keys(facts.extraKenmerken ?? {}).length;
  if (extraCount < 80) {
    facts.extraKenmerken = { ...(facts.extraKenmerken ?? {}), [label.replace(/\s+/g, " ").trim().slice(0, 80)]: text.slice(0, 200) };
  }
...
export function applyJsonLd(node: Record<string, unknown>, facts: ImportedListingFacts) {
  ...
  if (description && description.length > 40) facts.description ??= description.slice(0, 8_000);
...
export function applyJsonLdText(raw: string, facts: ImportedListingFacts) {
  try {
...
export function walkEmbeddedFields(value: unknown, facts: ImportedListingFacts, depth = 0) {
  ...
  for (const [key, raw] of Object.entries(record)) {
    ...
    if (mapped && !hasValue(facts[mapped])) {
      if (mapped === "description" || mapped === "neighborhood") {
        ...
        if (text && text.length > 40) (facts as Record<string, unknown>)[mapped] = text.slice(0, 8_000);
...
export function applyEmbeddedJsonText(raw: string, facts: ImportedListingFacts) {
  try {
    walkEmbeddedFields(JSON.parse(raw) as unknown, facts);
  } catch {
    /* ignore */
  }
}
...
export function preferLabelledAreas(facts: ImportedListingFacts) {
...
export function finalizeExtractedFacts(facts: ImportedListingFacts, visibleText?: string): ImportedListingFacts {
  ...
  if (merged.description) merged.description = merged.description.slice(0, 8_000);
  if (merged.sections) merged.sections = merged.sections.slice(0, 12);
  return merged;
}
